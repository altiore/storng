import {StoreWeb} from '@storng/store';
...
const storeWeb = new StoreWeb<StoreType>('storng');
...
describe('StoreWeb src/store.web.ts', () => {
  ...
  describe('updateData - обновление данных', () => {
    const updater = (s: any) => deepAssign(s, {
      data: {
        id: 'my-id'
      },
      loadingStatus: {
    ...
    it('После обновления данных, мы видим новые данные у первого подписчика', async () => {
      ...
      expect(subscriber1).to.nth(2).called.with({
        data: {
          id: 'my-id',
        },
        id: 'auth',
      ...
      expect(subscriber2).to.nth(2).called.with({
        data: {
          id: 'my-id',
        },
        id: 'auth',
